...
def find_pods(v1):
    """Find pods started by us or at least running paxos-peer"""
    ret = v1.list_pod_for_all_namespaces(watch=False)
    def pod_filter(p):
        return p.metadata.namespace == "default" and \
                len(p.spec.containers) == 1 and \
                p.spec.containers[0].image == 'local/paxos-peer'
...
def boot_pod_replica(v1, pod_spec, service_spec, name, leaders):
    """Boot a single pod of replica"""
    ...
    pod_spec['spec']['containers'][0]['ports'][1]['name']="%s-paxos"%name
    args = ['replica']
    ...
    service_spec['spec']['ports'][1]['targetPort'] = "%s-paxos"%name
    try:
...
def boot_pod_acceptor(v1, pod_spec, service_spec, name, leaders):
    """Boot a single pod of acceptor"""
    pod_spec = copy.deepcopy(pod_spec)
    # Create a pod spec for this pod.
    pod_spec['metadata']['name'] = name
    pod_spec['metadata']['labels']['app'] = name
...
def boot(args):
    ...
    with open(os.path.join(sys.path[0], 'pod-template.yml')) as f:
        ...
        replicas = ['rep%d'%i for i in range(num_replicas)]

        num_leaders = args.leaders
        leaders = ['lead%d'%i for i in range(num_leaders)]

        num_acceptors = args.acceptors
        acceptors = ['acc%d'%i for i in range(num_acceptors)]
...
def main():
    ...
    run_parser.add_argument('replicas', type=int, default=2, help='How many replicas?')
    run_parser.add_argument('leaders', type=int, default=2, help='How many leaders?')
    run_parser.add_argument('acceptors', type=int, default=3, help='How many acceptors?')
